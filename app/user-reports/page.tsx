...
import UserReport_DetailModal from "../components/modals/UserReport_DetailModal";
import Pagination from "../components/ui/Pagination";
import TableSkeleton from "../components/ui/TableSkeleton";
import CardSkeleton from "../components/ui/CardSkeleton";
import { AppDispatch, RootState } from "../store";
import { fetchUserReports, clearError, UserReport } from "../store/slices/userReportsSlice";
...
export default function UserReportsPage() {
  ...
  const { reports, loading, error, pagination, lastFetched } = useSelector(
    (state: RootState) => state.userReports
  );
  ...
  useEffect(() => {
    if (reports.length === 0 || isDataStale || pagination.page !== currentPage) {
      dispatch(fetchUserReports({ page: currentPage, limit: 20 }));
    }
  ...
  const handlePageChange = (page: number) => {
    setCurrentPage(page);
    dispatch(fetchUserReports({ page, limit: 20 }));
  };
    ...
    <DashboardLayout>
      <div className="w-full p-4 sm:p-5 lg:p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">User Reports</h1>
            <p className="text-gray-500 text-sm">User-submitted reports on other users</p>
          </div>
        </div>
        ...
        {loading && reports.length === 0 ? (
          ...
          <div className="bg-white rounded-lg border border-gray-200 py-12 text-center">
            ...
            <p className="text-gray-400 text-sm">Reports submitted by users on other users will appear here</p>
          </div>
        ) : (
          <>
            {/* Mobile Cards */}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    <th className="text-left font-medium text-gray-600 px-3 py-2.5">Reported User</th>
