...
export async function POST(req: NextRequest) {
  try {
    ...
    const publishedSnap = await xpEngineDocRef(XP_ENGINE_PUBLISHED_DOC_ID).get();
    if (!publishedSnap.exists) {
      return NextResponse.json({ error: "No published config to duplicate" }, { status: 404 });
    }
    ...
    const errorMessage = error instanceof Error ? error.message : "Failed to duplicate XP engine config";
